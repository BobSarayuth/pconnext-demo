...
def _dump_metadata(serde: SerializerProtocol, metadata: CheckpointMetadata) -> tuple[str, bytes]:
    return serde.dumps_typed(metadata)


def _load_metadata(serde: SerializerProtocol, data: dict) -> CheckpointMetadata:
    if b"metadata_type" in data:
        return serde.loads_typed((data[b"metadata_type"].decode(), data[b"metadata"]))
...
def _filter_keys(keys: List[str], before: Optional[RunnableConfig], limit: Optional[int]) -> list:
    ...
    if before and "configurable" in before:
        keys = [
            k
            for k in keys
            if _parse_redis_checkpoint_key(k.decode())["checkpoint_id"] < before["configurable"]["checkpoint_id"]  # type: ignore
        ]
...
def _parse_redis_checkpoint_data(
    ...
    metadata = _load_metadata(serde, data)
...
class AsyncRedisSaver(BaseCheckpointSaver):
        ...
        super().__init__(
            serde=JsonPlusSerializer(
                allowed_msgpack_modules=[
                    ("agentic.models.responsed", "MainResponded"),
                    ("agentic.models.responsed", "RespondedAttachment"),
                    ("agentic.models.responsed", "ActionType"),
                    ("agentic.models.responsed", "AttachmentType"),
                ]
            )
        )
        self.conn = conn
    ...
    @classmethod
    @asynccontextmanager
    async def from_conn_info(cls, *, host: str, port: int, db: int, password: str) -> AsyncIterator["AsyncRedisSaver"]:
        conn = None
        try:
    ...
    @classmethod
    @asynccontextmanager
    async def from_conn_uri(cls, *, uri: str) -> AsyncIterator["AsyncRedisSaver"]:
        conn = None
        try:
            conn = AsyncRedis.from_url(uri)
            yield AsyncRedisSaver(conn)
        finally:
            if conn:
                await conn.aclose()
    ...
    async def aput(
        ...
        metadata_type, serialized_metadata = _dump_metadata(self.serde, metadata)
        data = {
        ...
        if AGENTIC_CACHE_TTL > 0:
            await self.conn.expire(key, AGENTIC_CACHE_TTL)
    ...
    async def adel_tuple(self, checkpoint_id: str | list[str]) -> int:
        """Clear all checkpoints associated with a session."""
        prefixs = ["writes", "checkpoint"]
        if isinstance(checkpoint_id, str):
            checkpoint_id = [checkpoint_id]

        all_keys = 0
        for key in checkpoint_id:
            search_patterns = [f"{prefix}{REDIS_KEY_SEPARATOR}{key}{REDIS_KEY_SEPARATOR}*" for prefix in prefixs]

            # Scan for each pattern separately
    ...
    async def aflushdb(self) -> int:
        """Clear all checkpoints associated with a session."""
        return await self.conn.flushdb()
        ...
        *,
        ...
        for key in keys:
            ...
            if data and b"checkpoint" in data and b"metadata" in data:
                checkpoint_id = _parse_redis_checkpoint_key(key.decode())["checkpoint_id"]
                pending_writes = await self._aload_pending_writes(thread_id, checkpoint_ns, checkpoint_id)
                yield _parse_redis_checkpoint_data(self.serde, key.decode(), data, pending_writes=pending_writes)  # type: ignore
